...
# Resolve a feature directory for a branch-like identifier (STAYZ patch).
# 1. Exact match: specs/<name> exists.
# 2. Numeric-prefix match: name starts with NNN- and exactly one specs/NNN-*
#    directory exists — this lets per-session worktree branches carry a suffix
#    (e.g. 039-ru-certification-readiness-inbox) while resolving to the shared
#    feature directory (same branch cannot be checked out in two worktrees).
# Prints the absolute directory on success; returns 1 when nothing matches.
_feature_dir_for_branch() {
    local repo_root="$1" name="$2"
    [[ -z "$name" ]] && return 1
    if [[ -d "$repo_root/specs/$name" ]]; then
        printf '%s' "$repo_root/specs/$name"
        return 0
    fi
    if [[ "$name" =~ ^([0-9]{3})- ]]; then
        ...
        if [[ ${#matches[@]} -eq 1 ]]; then
            ...
            echo "WARNING: branch '$name' matches ${#matches[@]} specs/$num-* directories — falling back to feature.json" >&2
        fi
    fi
    return 1
}
...
get_feature_paths() {
    # --no-persist is accepted for backward compatibility with existing
    # callers, but path resolution never writes .specify/feature.json anymore
    # (STAYZ patch — see resolution priority comment below).
    ...
    # Detect git context (also exported as HAS_GIT for callers under set -u).
    local has_git=false git_branch=''
    ...
    # Resolve feature directory.  Priority (STAYZ patch — branch-first):
    #   1. SPECIFY_FEATURE_DIRECTORY env var (explicit override)
    #   2. SPECIFY_FEATURE env var, if specs/<value> exists
    #   3. Git branch, if specs/<branch> exists (per-checkout state; the
    #      constitution mandates branch name == spec directory name)
    #   4. .specify/feature.json "feature_directory" key (fallback for
    #      non-git contexts and detached/main checkouts)
    #   5. Error — no feature context available
    # feature.json is a single global pointer shared by every concurrent
    # session in this checkout, so it must never override branch context —
    # parallel sessions were clobbering each other's feature state.
    # For the same reason plain path resolution never WRITES feature.json
    # anymore (writes happen only in create-new-feature/specify).
    local feature_dir
    if [[ -n "${SPECIFY_FEATURE_DIRECTORY:-}" ]]; then
        feature_dir="$SPECIFY_FEATURE_DIRECTORY"
        # Normalize relative paths to absolute under repo root
        [[ "$feature_dir" != /* ]] && feature_dir="$repo_root/$feature_dir"
    elif [[ -n "$current_branch" ]] && feature_dir=$(_feature_dir_for_branch "$repo_root" "$current_branch"); then
        :
    elif [[ -n "$git_branch" ]] && feature_dir=$(_feature_dir_for_branch "$repo_root" "$git_branch"); then
